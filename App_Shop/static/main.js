...
cartBtn.forEach((btn) => {
    btn.addEventListener('click', (e) => {
        ...
        if (existingItem) {
            ...
            const updateData=cartLi.querySelector("p");
            updateData.innerHTML=`${existingItem.product_quantity} X <span class="price">${existingItem.product_price}=${existingItem.product_quantity*existingItem.product_price}</span>`
            getTotol();
        }
        else {
...
const updatCartList = (product_id,product_name, product_price, product_quantity, product_img) => {
    ...
    if (cart == null) {
        ...
        <p>${product_quantity} X <span class="price">${product_price}=${product_quantity*product_price}</span></p>
        `
